...
function getIntersectionList(mainshape,p0,p1){
	var a = 0;
	var crossArray =[];
	while(a < mainshape.length)
	{
	   if(a < mainshape.length - 1)
	   {
		  p = checkIntersection(
		  p0,p1,
		  new Point(mainshape[a].x,mainshape[a].y),
		  new Point(mainshape[a + 1].x,mainshape[a + 1].y));
	   }
	   else
	   {
		  p = checkIntersection(
		  p0,p1,
		  new Point(mainshape[a].x,mainshape[a].y),
		  new Point(mainshape[0].x,mainshape[0].y));
	   }
	   if(p != null)
	   {
		  crossArray.push(p);
...
function appendDistance(_center,_point,_distance){
	var _leng = Math.sqrt(((_point.x - _center.x)*(_point.x -_center.x)) + ((_point.y -_center.y)*(_point.y -_center.y)));
	var dx = [_center.x,_center.y];
	var dy = [_point.x,_point.y];
	var _rad = angleBetweenPoints(dx,dy);
	 
	//if(_rad <0) _rad = _rad + 2*3.14;
	var _x = (_leng + _distance) * Math.cos(_rad) + _center.x;
	var _y = (_leng + _distance) * Math.sin(_rad) + _center.y;
	return new Point(_x,_y);
}

function getPointByCenterAndDistance(_center,_rad,_distance){
	//var _rad = angleBetweenPoints(dx,dy);
	var _x = (_distance) * Math.cos(_rad) + _center.x;
	var _y = (_distance) * Math.sin(_rad) + _center.y;
	return new Point(_x,_y);
}

function getPointByAngle(_center,_rad,_distance){
	var _x = (_distance) * Math.cos(_rad) + _center.x;
	var _y = (_distance) * Math.sin(_rad) + _center.y;
	return new Point(_x,_y);
}
